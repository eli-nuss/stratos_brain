...
The Daily Brief v3 is an AI-generated market summary that surfaces the best trading setups using a "Candidate Generation → Re-ranking" architecture. It is generated by the `daily-brief-api-v3` edge function using the Gemini-2.0-Flash model.

**Architecture:**
- **Stage 1 - Candidate Generation**: Fetches top 20 signals per setup type from `setup_signals` table (~220 candidates)
- **Stage 2 - Bucketing & Scoring**: Groups candidates into 3 categories with composite scores (ai_direction_score + setup_purity_score)
- **Stage 3 - AI Re-ranking**: Gemini selects top 15 picks per category with conviction ratings and one-liners

**Three Categories:**
1. **Momentum Breakouts**: New trends, breakouts, relative strength (weinstein_stage2_breakout, donchian_55_breakout, rs_breakout, breakout_confirmed, gap_up_momentum)
2. **Trend Continuation**: Riding existing trends, pullbacks to support (golden_cross, adx_holy_grail, acceleration_turn, trend_pullback_50ma)
...
|---------|------|---------|
| 2.5 | Jan 23, 2026 | **ETF Model Portfolio Integration**: Added 6 ETFs (GDXJ, SILJ, URA, COPX, GRID, PLTM) to the main `assets` table with full `daily_bars` and `daily_features` data (5 years historical). ETFs now appear in Model Portfolio search via `mv_dashboard_all_assets` and support full analytics. Updated `control-api` to include ETF search results from `v_etf_overview` as fallback. |
