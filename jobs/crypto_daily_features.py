#!/usr/bin/env python3
"""
Crypto Daily Features Calculation Job

Calculates technical features for all crypto assets for a given date.
Designed to run after crypto_daily_ohlcv.py completes.

Usage:
    python jobs/crypto_daily_features.py --date 2026-01-06
    python jobs/crypto_daily_features.py  # defaults to yesterday
"""
...
def get_stale_assets(target_date: str) -> list:
    """Get crypto assets that need feature calculation for target_date."""
    conn = get_connection()
...
def compute_features(bars: pd.DataFrame) -> pd.DataFrame:
    """Compute technical features for a single crypto asset."""
...
def build_record(row: pd.Series, asset_id: int, target_date: str) -> dict:
    ...
    feature_cols = [
        ...
        'rsi_14', 'bb_middle', 'bb_upper', 'bb_lower', 'bb_width', 'bb_pct', 'bb_width_pctile',
        ...
        # Setup-related boolean flags (required for setup scanner)
        'breakout_confirmed_up', 'breakout_confirmed_down',
        'accel_turn_up', 'accel_turn_down', 'accel_z_20_prev',
        'rs_breakout', 'rs_velocity', 'rs_acceleration', 'rs_vs_benchmark', 'rs_roc_20',
...
def update_latest_date(target_date: str, asset_type: str = 'crypto'):
...
def main():
    parser = argparse.ArgumentParser(description='Crypto Daily Features Calculation')
    ...
    logger.info("=" * 60)
    logger.info("CRYPTO DAILY FEATURES CALCULATION")
    logger.info(f"Target Date: {target_date}")
    logger.info(f"Workers: {args.workers}")
    logger.info("=" * 60)
    
    # Get stale assets
    logger.info("Fetching crypto assets needing features...")
    stale_assets = get_stale_assets(target_date)
    total = len(stale_assets)
    
    logger.info(f"Found {total} crypto assets needing features for {target_date}")
    ...
    logger.info("=" * 60)
    logger.info("CRYPTO FEATURES CALCULATION COMPLETE")
    logger.info("=" * 60)
    ...
    if success > 0:
        update_latest_date(target_date, 'crypto')
