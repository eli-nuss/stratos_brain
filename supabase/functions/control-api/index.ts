...
If asked about something not in the data, acknowledge the limitation.`
      ...
      case req.method === 'POST' && path.endsWith('/save') && path.includes('/dashboard/memo-status/'): {
        ...
        const body = await req.json()
        const { asset_id, file_url, file_name } = body
        ...
        const finalFileName = file_name || `memo_${taskId}.md`
        
        // Upload to Supabase Storage
        const storagePath = `memos/${asset_id}/${finalFileName}`
        const { data: uploadData, error: uploadError } = await supabase.storage
          .from('asset-files')
        ...
        // Update the asset_files record with the new URL
        ...
        if (updateError || !updateData || updateData.length === 0) {
          console.error('Database update error or no matching record:', updateError)
