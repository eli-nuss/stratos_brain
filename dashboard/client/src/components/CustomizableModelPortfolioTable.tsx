...
import { useModelPortfolioAssets, useModelPortfolio } from "@/hooks/useModelPortfolio";
...
interface CustomizableModelPortfolioTableProps {
...
export default function CustomizableModelPortfolioTable({ onAssetClick }: CustomizableModelPortfolioTableProps) {
  const { assets, isLoading, mutate: mutateAssets } = useModelPortfolioAssets();
  const { addToModelPortfolio, mutate: mutatePortfolio } = useModelPortfolio();
  const { 
    config, 
    getVisibleColumns, 
    getAvailableColumns, 
  ...
  const { sortBy, sortOrder, handleSort } = useSortPreferences("model-portfolio");
          ...
          <table className="w-full text-sm text-left min-w-[1200px]">
            ...
            <tbody>
              ...
              {!isLoading && safeAssets.length > 0 && (
                <TableSummaryRows assets={safeAssets} visibleColumns={visibleColumns} listName="Model Portfolio" />
              )}
              {isLoading ? (
                ...
                <tr><td colSpan={colCount} className="px-2 py-4 text-center text-muted-foreground">
                  Model portfolio is empty. Use the search above to add assets.
                </td></tr>
              ) : (
