#!/usr/bin/env python3
"""
E2B Parallel Parameter Optimizer v2 for Stratos Brain

Improvements:
- Smaller chunks (50 combinations per sandbox)
...
# Optimized sandbox code - more compact
SANDBOX_CODE = '''
import subprocess
...
def run_sandbox(setup_name: str, param_chunk: List[tuple], chunk_id: int, retry: int = 0) -> Dict:
    """Run optimization in a single e2b sandbox with retry logic"""
    print(f"[{setup_name}][{chunk_id}] Starting ({len(param_chunk)} combos)...")
    ...
    try:
        ...
        if 'RESULT_JSON_START' in output_text and 'RESULT_JSON_END' in output_text:
            ...
            if result_data.get('best'):
                print(f"[{setup_name}][{chunk_id}] Done - Best PF={result_data['best']['profit_factor']}")
            else:
            ...
            # Retry once
            if retry < 1:
                print(f"[{setup_name}][{chunk_id}] Retrying...")
                time.sleep(2)
                return run_sandbox(setup_name, param_chunk, chunk_id, retry + 1)
            
            return {'setup': setup_name, 'chunk_id': chunk_id, 'error': 'No JSON output'}
...
def optimize_setup(setup_name: str, max_sandboxes: int = 12) -> Dict:
    """Optimize a setup using parallel sandboxes"""
    print(f"\n{'='*50}")
    ...
    # Use smaller chunks - 30 combos per sandbox
    chunk_size = 30
    chunks = [all_combos[i:i+chunk_size] for i in range(0, total, chunk_size)]
    num_chunks = len(chunks)
    print(f"Splitting into {num_chunks} chunks of ~{chunk_size} each")
    
    start_time = time.time()
    all_results = []
    
    # Run chunks in parallel
    with ThreadPoolExecutor(max_workers=min(max_sandboxes, num_chunks)) as executor:
        futures = {executor.submit(run_sandbox, setup_name, chunk, i): i for i, chunk in enumerate(chunks)}
        ...
        for future in as_completed(futures):
            try:
                ...
                print(f"[{setup_name}] Chunk failed: {e}")
...
def main():
    print("="*60)
    print("E2B PARALLEL OPTIMIZER v2")
    print("="*60)
    ...
    print(f"  TOTAL: {total_all} combinations")
    print("="*60)
    
    start_time = time.time()
    all_results = []
    
    # Run setups in parallel (4 at a time, each with up to 12 sandboxes)
    with ThreadPoolExecutor(max_workers=4) as executor:
        futures = {executor.submit(optimize_setup, setup, 12): setup for setup in setups}
