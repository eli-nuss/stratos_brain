#!/usr/bin/env python3
"""
Global Equity Daily Features Calculation

Calculates technical features for all global_equity assets.
Based on equity_daily_features.py but for global_equity asset type.

Usage:
    python scripts/global_equity_features.py --date 2026-01-15
    python scripts/global_equity_features.py  # defaults to latest date with bars
"""
...
ANN_FACTOR = 252  # Assume 252 trading days/year
...
def get_all_dates_needing_features() -> list:
    """Get all dates that have bars but no features for global_equity."""
    conn = get_connection()
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        cur.execute("""
            SELECT DISTINCT db.date
            FROM daily_bars db
            JOIN assets a ON a.asset_id = db.asset_id
            WHERE a.asset_type = %s
              AND a.is_active = true
              AND NOT EXISTS (
                  SELECT 1 FROM daily_features df 
                  WHERE df.asset_id = db.asset_id AND df.date = db.date
              )
            ORDER BY db.date
        """, (ASSET_TYPE,))
        return [row['date'].strftime('%Y-%m-%d') for row in cur.fetchall()]


def get_stale_assets(target_date: str) -> list:
    """Get global_equity assets that need feature calculation for target_date."""
    conn = get_connection()
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        cur.execute("""
...
def build_record(row: pd.Series, asset_id: int, target_date: str) -> dict:
    ...
    feature_cols = [
        ...
        'atr_14', 'kc_upper', 'kc_lower', 'squeeze_flag', 'squeeze_release',
...
def main():
    parser = argparse.ArgumentParser(description='Global Equity Daily Features Calculation')
    parser.add_argument('--date', type=str, help='Target date (YYYY-MM-DD). Defaults to all missing dates.')
    parser.add_argument('--workers', type=int, default=8, help='Number of parallel workers')
    parser.add_argument('--batch-size', type=int, default=100, help='Batch size for DB writes')
    args = parser.parse_args()
    
    logger.info("=" * 60)
    logger.info("GLOBAL EQUITY DAILY FEATURES CALCULATION")
    logger.info(f"Workers: {args.workers}")
    logger.info("=" * 60)
    
    # Get dates needing features
    if args.date:
        dates_to_process = [args.date]
    else:
        dates_to_process = get_all_dates_needing_features()
    
    logger.info(f"Found {len(dates_to_process)} dates needing feature calculation")
    ...
    for target_date in dates_to_process:
        ...
        # Get stale assets for this date
        stale_assets = get_stale_assets(target_date)
        total = len(stale_assets)
        
        if total == 0:
            logger.info(f"  No assets need features for {target_date}")
            continue
        
        logger.info(f"  Found {total} assets needing features")
        ...
        with ThreadPoolExecutor(max_workers=args.workers) as executor:
            futures = {executor.submit(process_asset, asset, target_date): asset for asset in stale_assets}
            ...
            for future in as_completed(futures):
                ...
                if result['status'] == 'success':
                    success += 1
                    records_to_write.append(result['record'])
                    
                    # Write in batches
                    if len(records_to_write) >= args.batch_size:
                        write_features_batch(records_to_write)
                        records_to_write = []
                    ...
                    errors += 1
                    logger.warning(f"  Error for asset {result['asset_id']}: {result.get('error', 'unknown')}")
        
        # Write remaining records
        if records_to_write:
            write_features_batch(records_to_write)
        
        logger.info(f"  Date {target_date}: {success} success, {skipped} skipped, {errors} errors")
        total_success += success
        total_skipped += skipped
        total_errors += errors
    
    logger.info("=" * 60)
    logger.info(f"COMPLETE: {total_success} success, {total_skipped} skipped, {total_errors} errors")
    logger.info("=" * 60)
