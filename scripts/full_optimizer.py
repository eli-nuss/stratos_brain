#!/usr/bin/env python3
"""
Full Parameter Optimizer for Stratos Brain
Optimizes both entry conditions and exit parameters for each setup

Uses grid search to find optimal parameters for:
- Entry thresholds (RSI, MA distances, volume, etc.)
- Exit parameters (stops, targets, time limits, trailing stops)
"""
...
OPTIMIZATION_GRIDS = {
    'oversold_bounce': {
        'entry': {
            ...
            'ma_dist_threshold': [-0.05, -0.08, -0.10, -0.12],
        },
        'exit': {
            'target_ma_dist': [-0.02, 0, 0.02],
    ...
    'vcp_squeeze': {
        'entry': {
            'bb_width_pctile_threshold': [10, 15, 20, 25],
            'rsi_min': [35, 40, 45],
            'rsi_max': [65, 70, 75],
        },
        'exit': {
    ...
    'trend_pullback_50ma': {
        'entry': {
            'ma_dist_min': [-0.05, -0.03, -0.02],
            'ma_dist_max': [0.01, 0.02, 0.03],
    ...
    'golden_cross': {
        'entry': {
            'ma_dist_min': [-0.03, -0.02, -0.01],
            'ma_dist_max': [0.03, 0.05, 0.07],
            'rsi_min': [40, 45, 50],
            'rsi_max': [60, 65, 70],
        },
        'exit': {
            'breakdown_ma_dist_50': [-0.02, -0.03, -0.05],
    ...
    'breakout_confirmed': {
        ...
        'exit': {
            ...
            'trailing_activation_pct': [0.08, 0.12, 0.15],
...
def get_universe(conn, limit: int = 500) -> List[Tuple[int, str]]:
    ...
    cur.execute('''
        ...
        AND rl.avg_dollar_volume > 5000000
...
def optimize_setup(
    ...
    for entry_combo in entry_combos:
        ...
        for exit_combo in exit_combos:
            ...
            if result['score'] > best_score and result['trades'] >= 50:
...
def main():
    parser = argparse.ArgumentParser(description='Full Parameter Optimizer')
    parser.add_argument('--setup', type=str, help='Specific setup to optimize')
    parser.add_argument('--universe', type=int, default=500, help='Number of assets')
    ...
    sorted_results = sorted(results.items(), key=lambda x: x[1]['best_result']['score'], reverse=True)
