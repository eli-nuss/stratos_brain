...
def run_sandbox(setup_name: str, param_chunk: List[tuple], chunk_id: int) -> Dict:
    ...
    try:
        ...
        if 'RESULT_JSON_START' in output_text and 'RESULT_JSON_END' in output_text:
            ...
            print(f"Output: {output_text[:1000]}")
            if result.logs and result.logs.stderr:
                print(f"Stderr: {result.logs.stderr}")
            sbx.kill()
            return {'setup': setup_name, 'chunk_id': chunk_id, 'error': 'No JSON output', 'output': output_text[:2000]}
...
def optimize_setup_parallel(setup_name: str, max_sandboxes: int = 10) -> Dict:
    ...
    if chunk_size < 20:  # Minimum chunk size to avoid too many sandboxes
        chunk_size = min(20, total_combos)
...
def main():
    ...
    # Run all setups in parallel with up to 8 sandboxes each
    all_results = []
    
    # Process setups in parallel batches
    with ThreadPoolExecutor(max_workers=4) as executor:
        futures = {executor.submit(optimize_setup_parallel, setup, 8): setup for setup in setups}
