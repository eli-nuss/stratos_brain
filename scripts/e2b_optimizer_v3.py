#!/usr/bin/env python3
"""
E2B Parameter Optimizer v3 - Rate Limited Version

Runs sandboxes sequentially to avoid rate limits.
Uses larger chunks to minimize sandbox count.
"""
...
def run_sandbox_with_retry(setup_name: str, param_combos: List[tuple], max_retries: int = 3) -> Dict:
    """Run a sandbox with retry and rate limit handling"""
    for attempt in range(max_retries):
        try:
            print(f"[{setup_name}] Creating sandbox (attempt {attempt + 1})...")
            sbx = Sandbox.create()
            ...
            print(f"[{setup_name}] Running {len(param_combos)} combinations...")
            ...
            if 'RESULT_JSON_START' in output_text and 'RESULT_JSON_END' in output_text:
                ...
                if result_data.get('best'):
                    print(f"[{setup_name}] SUCCESS - Best PF={result_data['best']['profit_factor']}, WR={result_data['best']['win_rate']}%")
                else:
                    print(f"[{setup_name}] Completed but no valid results (min 30 trades)")
                
                sbx.kill()
                return result_data
            else:
                stderr = '\n'.join(result.logs.stderr) if result.logs and result.logs.stderr else ''
                print(f"[{setup_name}] No JSON output found")
                if 'Error' in stderr:
            ...
            if '429' in error_str:
                ...
                print(f"[{setup_name}] Rate limited, waiting {wait_time}s...")
                time.sleep(wait_time)
            else:
                print(f"[{setup_name}] Error: {error_str[:100]}")
                time.sleep(5)
    
    return {'setup': setup_name, 'error': 'Max retries exceeded'}


def main():
    print("="*60)
    print("E2B PARAMETER OPTIMIZER v3 (Rate Limited)")
    print("="*60)
    ...
    # Process setups with limited concurrency (2 at a time)
    with ThreadPoolExecutor(max_workers=2) as executor:
        futures = {}
        for setup in setups:
            combos = generate_param_combinations(setup)
            futures[executor.submit(run_sandbox_with_retry, setup, combos)] = setup
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                print(f"[{setup}] Saved to {output_file}")
            except Exception as e:
